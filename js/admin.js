    ...
    */
    ...
    var SBSettings = {
        ...
        automations: {
            ...
            add: function () {
                let id = SBF.random();
                let name = `${sb_('Item')} ${automations_area_nav.find('li').length + 1}`;
                this.updateActiveItem();
            ...
            populate: function (type = false) {
                ...
                switch (type) {
                    case 'emails':
                        ...
                        break;
                    case 'popups':
                        code = `<h2>${sb_('Title')}</h2><div class="sb-input-setting sb-type-text"><div><input data-id="title" type="text"></div></div><h2>${sb_('Profile image')}</h2><div data-type="upload-image" class="sb-input-setting sb-type-upload-image"><div class="input"><div data-id="profile_image" class="image"><i class="sb-icon-close"></i></div></div></div>`;
                        break;
                    case 'design':
                    ...
                    for (var i = 0; i < items.length; i++) {
                        if (items[i]['id'] == id) {
                            ...
                            automations_area.find('.sb-automation-values').find('input,textarea,[data-type="upload-image"] .image').each(function () {
                                items[i][$(this).attr('data-id')] = $(this).hasClass('image') && $(this)[0].hasAttribute('data-value') ? $(this).attr('data-value') : $(this).val();
                            });
    ...
    */
    ...
    var SBUsers = {
        ...
        populate: function (response) {
            ...
            this.updateMenuItem('set', users_table_menu.find('.sb-active').data('type'), count);
        },
    ...
    var SBConversations = {
        ...
        openConversation: function (conversation_id, user_id = false, scroll = true) {
            if (user_id === false && conversation_id) {
                ...
                updateUsersActivity();
                this.startRealTime();
                if (SBF.getURL('conversation') != conversation_id) window.history.pushState('', '', '?conversation=' + conversation_id);
            }
        },
                    ...
                    if (response.length) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                if (!SBChat.tab_active && item['conversation_status_code'] == 2 && (!SBF.isAgent(item['message_user_type']) || 'bot-unknow-notify' in payload)) {
                                    if (this.desktop_notifications) {
                                        ...
                                        if ('bot-unknow-notify' in payload) {
                                            users[user_id].getFullConversation(conversation_id, (response) => {
                                                SBChat.desktopNotification(user_details[0], response.getLastUserMessage().get('message'), user_details[1], conversation_id, user_id);
                                            });
                                        } else {
                                    ...
                                    if (['a', 'c', 'ic'].includes(SB_ADMIN_SETTINGS['sounds'])) {
                                        SBChat.audio.play();
                                    }
                                }
        ...
        getListCode: function (conversation, status, label = false) {
            ...
            return `<li data-user-id="${conversation['user_id']}" data-conversation-id="${conversation['conversation_id']}" data-conversation-status="${status}"${!SBF.null(conversation['conversation_source']) ? ` data-conversation-source="${conversation['conversation_source']}"` : ''}>` + (label && status == 2 ? '<span>' + sb_('New') + '</span>' : '') + `<div class="sb-profile"><img src="${conversation['profile_image']}"><span class="sb-name">${conversation['first_name']} ${conversation['last_name']}</span><span class="sb-time">${SBF.beautifyTime(conversation['creation_time'])}</span></div><p>${message}</p></li>`;
        },
        ...
        attachments: function () {
            if (attachments_panel.length) {
                ...
                $(attachments_panel).html(code == '' ? '' : `<h3>${sb_('Attachments')}</h3><div class="sb-list-links">${code}</div>`);
            }
        },
    ...
    var SBProfile = {
        ...
        show: function (user_id) {
            ...
            activeUser().update(() => {
                ...
                users[user_id] = activeUser();
                if (SBF.getURL('user') != user_id) window.history.pushState('', '', '?user=' + user_id);
            });
        },
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : source, sb_('Source'));
                }
            }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger' };
                let updates = false;
        ...
        */
        ...
        $(header).on('click', ' .sb-admin-nav a', function () {
            ...
            if (url_area != slug && ((slug == 'conversations' && !SBF.getURL('conversation')) || (slug == 'users' && !SBF.getURL('user')) || (slug == 'settings' && !SBF.getURL('setting')) || (slug == 'reports' && !SBF.getURL('report')))) window.history.pushState('', '', '?area=' + slug);
        });
        ...
        */
            ...
            if (SBApps.messenger.check(SBChat.conversation)) {
                SBApps.messenger.send(activeUser().getExtra('facebook-id').value, SBChat.conversation.get('extra'), response['message'], response['attachments'], (response) => {
                    if ('error' in response) {
                        SBChat.deleteMessage(SBChat.conversation.getLastMessage().id);
        ...
        $(document).on('SBNewMessagesReceived', function () {
            setTimeout(function () {
                conversations_area.find('.sb-conversation .sb-top .sb-status-typing').remove();
            }, 300);
            SBConversations.update();
        ...
        $(profile_edit_box).on('click', '.sb-save', function () {
            ...
            if (!$(this).sbLoading()) {
                ...
                if (SBProfile.errors(profile_edit_box)) {
                    SBProfile.showErrorMessage(profile_edit_box, ['user', 'agent', 'admin'].includes(profile_edit_box.find('#user_type :selected').val()) ? 'First name, last name, and a valid email are required.' : 'First name and last name are required.');
                    $(this).sbLoading(false);
                    return;
                }
                ...
                if (type == 'message') {
                    ...
                    SBF.ajax({
                        ...
                        if (response[slug].length) {
                            ...
                            SBForm.showErrorMessage(direct_message_box, 'No users found.');
                        }
                    });
                }
                $(this).sbLoading(true);
        ...
        */
        ...
        $(settings_area).on('click', ' > .sb-tab > .sb-nav [id]', function () {
            ...
            if (SBF.getURL('setting') != id) window.history.pushState('', '', '?setting=' + id);
        });
        ...
        $(settings_area).on('click', '#tab-translations', function () {
            SBF.ajax({
                ...
                for (var key in translations) {
                    ...
                    code_nav += `<li data-code="${key}"><img src="${SB_URL}/media/flags/languages/${key}.png" />${translations[key]['name']}</li>`;
        ...
        */
        ...
        $(reports_area).on('click', '.sb-nav [id]', function () {
            ...
            if (SBF.getURL('report') != id) window.history.pushState('', '', '?report=' + id);
        });
