...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger'];
    ...
    switch ($action) {
    	case 'connect':
            ...
            if ($connection->connect_error) {
                ...
                return new SBError('db-error', 'sb_external_db', $connection->connect_error);
            }
...
function sb_get_active_user($login_data = false, $database = false, $login_app = false) {
    global $SB_LOGIN;
    $return = false;
    if (isset($SB_LOGIN)) {
        $return = $SB_LOGIN;
    ...
    if ($database && $return && isset($return['id'])) {
        $active_user = sb_db_get('SELECT profile_image, first_name, last_name, email, user_type FROM sb_users WHERE id = ' . $return['id']);
        if ($active_user) {
...
function sb_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    if (!empty($settings_extra['phone']) && intval(sb_db_get('SELECT COUNT(*) as count FROM sb_users_data WHERE slug = "phone" AND value = "' . $settings_extra['phone'][0] . '"')['count']) > 0) {
...
function sb_update_user($user_id, $settings, $settings_extra = [], $hash_password = true) {
    ...
    if (!empty($settings_extra['phone']) && intval(sb_db_get('SELECT COUNT(*) as count FROM sb_users_data WHERE slug = "phone" AND value = "' . $settings_extra['phone'][0] . '" AND user_id <> ' . $user_id)['count']) > 0) {
        return new SBValidationError('duplicate-phone');
    }
    ...
    $query_final = 'UPDATE sb_users SET profile_image = "' . sb_db_escape($profile_image) . '", last_name = "' . sb_db_escape($last_name) . '", user_type = "' . $user_type . '", email = ' . (strlen($email) == 0 ? 'NULL' : '"' . sb_db_escape($email) . '"') . ', department = ' . $department . $query . ' WHERE id = "' . $user_id . '"';
...
function sb_get_conversation($user_id = false, $conversation_id) {
    ...
    if (isset($messages) && is_array($messages)) {
        ...
        if (sb_is_agent() && !sb_get_setting('disable-notes')) {
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $routing = false, $source = false, $extra = false) {
    ...
    $conversation_id = sb_db_query('INSERT INTO sb_conversations(user_id, title, status_code, creation_time, department, agent_id, source, extra) VALUES (' . $user_id . ', "' . sb_db_escape(htmlspecialchars(ucfirst($title), ENT_NOQUOTES)) . '", "' . ($status_code == -1 || $status_code === false || $status_code === '' ? 2 : $status_code) . '", "' . gmdate('Y-m-d H:i:s') . '", ' . (sb_isset_num($department) ? $department : 'NULL') . ', ' . (sb_isset_num($agent_id) ? $agent_id : 'NULL') . ', ' . ($source ? '"' . $source . '"' : 'NULL') . ', ' . ($extra ? '"' . sb_db_escape(htmlspecialchars($extra)) . '"' : 'NULL') . ')', true);
...
function sb_transcript($conversation_id, $type = false) {
    ...
    if (!sb_is_agent() && sb_isset(sb_db_get('SELECT user_id FROM sb_conversations WHERE id = ' . $conversation_id), 'user_id') != sb_get_active_user()['id']) return new SBError('security-error', 'sb_transcript');
    if ($type === false) $type = sb_get_setting('transcript-type');
    if ($type == 'csv') {
...
function sb_direct_message($user_ids, $message, $subject = false) {
    ...
    $message_db = sb_db_escape(htmlspecialchars(sb_merge_fields($message), ENT_NOQUOTES));
...
function sb_send_message($sender_id = -1, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false, $recipient_id = false) {
    ...
    if ($sender_id != -1) {
        ...
        if ($security) {
            ...
            $response = sb_db_query('INSERT INTO sb_messages(user_id, message, creation_time, status_code, attachments, payload, conversation_id) VALUES ("' . $sender_id . '", "' . sb_db_escape($message) . '", "' . gmdate('Y-m-d H:i:s') . '", 0, "' . $attachments_json . '", "' . ($payload != false ? sb_db_json_enconde($payload) : '') . '", "' . $conversation_id . '")', true);
            ...
            if ($conversation_status_code != 'skip') {
                if ($conversation_status_code == -1 || $conversation_status_code === false || !in_array($conversation_status_code, [0, 1, 2, 3, 4])) {
                    ...
                    $conversation_status_code = $is_sender_agent && sb_isset($sender, 'user_type') != 'bot' && !$cookie_unknow_notify ? 1 : (defined('SB_DIALOGFLOW') && (sb_get_setting('bot-unknow-notify') || sb_get_multi_setting('dialogflow-human-takeover', 'dialogflow-human-takeover-active')) && !$cookie_unknow_notify ? 1 : 2);
                }
...
function sb_init_translations() {
    ...
    if (!empty($SB_LANGUAGE) && $SB_LANGUAGE[0] !='en') {
        ...
        if ((!$language && !$admin_lang) || ($admin && !sb_get_setting('admin-auto-translations') && !$admin_lang) || (!$admin && !isset($_GET['lang']) && !sb_get_setting('front-auto-translations'))) {
            $SB_TRANSLATIONS = false;
        } else {
            switch($language) {
                case 'nn':
...
function sb_get_user_language($user_id = false) {
    if ($user_id) {
        ...
        return $language ? $language : sb_get_user_extra($user_id, 'language', 'en');
    }
    global $SB_LANGUAGE;
    if (empty($SB_LANGUAGE)) {
        $language = isset($_GET['lang']) ? $_GET['lang'] : strtolower(substr(sb_isset($_SERVER, 'HTTP_ACCEPT_LANGUAGE', '  '), 0, 2));
        return $language != 'en' ? $language : false;
    }
    return $SB_LANGUAGE[0] != 'en' ? $SB_LANGUAGE[0] : false;
}
...
function sb_get_front_settings() {
    ...
    $return = [
        ...
        'messenger' => defined('SB_MESSENGER') && sb_get_setting('messenger-active'),
        'pusher' => $pusher,
...
function sb_get_block_setting($value) {
    switch ($value)  {
        ...
        case 'welcome':
            ...
            return ['active' => $settings['welcome-active'], 'message' => sb_rich_value($settings['welcome-msg'], true, true, true), 'open' => $settings['welcome-open'], 'sound' => $settings['welcome-sound']];
        case 'follow':
...
function sb_save_articles($articles, $categories = false, $translations = false) {
    if ($categories) sb_save_external_setting('articles-categories', $categories);
    if ($translations) {
        $db = '';
...
function sb_article_ratings($article_id, $rating = false) {
    ...
    if ($rating) {
        if (empty($ratings)) {
            return sb_db_query('INSERT INTO sb_reports (name, value, creation_time, external_id, extra) VALUES ("article-ratings", "[' . $rating . ']", "' . gmdate('Y-m-d') . '", NULL, "' . $article_id . '")');
        }  else {
...
function sb_email_create($recipient_id, $sender_name, $sender_profile_image, $message, $attachments = [], $department = false, $conversation_id = false) {
    ...
    } else if ($recipient_id == -1 || $recipient_id == 'agents' || strpos($recipient_id, 'department-') !== false) {
    ...
    $email = sb_email_create_content($settings['email-' . $suffix . '-subject'], $settings['email-' . $suffix . '-content'], $attachments, ['message' => $message, 'recipient_name' => $recipient_name, 'sender_name' => $sender_name, 'sender_profile_image' => $sender_profile_image, 'conversation_id' => $conversation_id]);
...
function sb_email_piping() {
    if (date('i') == sb_get_external_setting('cron-email-piping')) return false;
                ...
                foreach ($emails as $email_number) {
                    ...
                    if ($all_emails || $follow_up) {
                        ...
                        if (!in_array($id, $history)) {
                            $from_email = strpos($from, '<') ? trim(substr($from, strpos($from, '<') + 1, -1)) : $from;
                            $sender = sb_db_get('SELECT * FROM sb_users WHERE email = "' . $from_email . '" LIMIT 1');
                            if (!$sender && !$follow_up) {
                                $from_name = strpos($from, '<') && strpos($from, '=') === false && strpos($from, '?') === false ? trim(substr($from, 0, strpos($from, '<'))) : '';
                            ...
                            if ($sender && ($follow_up || !sb_is_agent($sender))) {
                                ...
                                if (strpos($message, ' | SB')) {
                                    $position = strpos($message, ' | SB');
                                    $part_1 = substr($message, 0, $position);
                                    $part_1 = substr($part_1, 0, strripos($part_1, PHP_EOL));
                                    $part_2 = substr($message, $position);
                                    $part_2 = substr($part_2, strpos($part_2, PHP_EOL));
                                    $message =  $part_1 . $part_2;
                                }
                                if (strpos($host, 'gmail')) {
                                    $message = str_replace('wrote:' . PHP_EOL, '', $message);
                                }
                                ...
                                // Send message
                                if (!empty($message)) {
                                    if (!$follow_up) $conversation_id = sb_get_last_conversation_id_or_create($sender['id']);
                                    $recipient = get_user_from_conversation($conversation_id, !$agent);
                                    sb_send_message($sender['id'], $conversation_id, $message, [], ($agent ? 1 : 2));
                                    ...
                                    if (isset($recipient['id']) && !sb_is_user_online($recipient['id'])) {
                                        ...
                                        if (($agent && sb_get_setting('sms-active-users')) || (!$agent && sb_get_setting('sms-active-agents'))) {
                                            $phone = sb_get_user_extra($recipient['id'], 'phone');
                                            if ($phone) {
                                                sb_send_sms($message, $phone);
                                            }
                                        }
                                    }
                                }
...
function sb_push_notification($title = '', $message = '', $icon = '', $interest = false, $conversation_id = false) {
    ...
    } else if (strpos($interest, 'department-') !== false) {
        $agents = sb_db_get('SELECT id FROM sb_users WHERE department = ' . substr($interest, 11) . ' AND (user_type = "agent" OR user_type = "admin")', false);
        $interest = [];
        for ($i = 0; $i < count($agents); $i++) {
    ...
    $query = ',"web":{"notification":{"title":"' . str_replace('"', '', $title) . '","body":"' . str_replace('"', '', sb_clear_text_formatting(preg_replace('/\r|\n/', ' ', $message))) . '","icon":"' . $icon . '"' . (empty($link) ? '' : ',"deep_link":"' . $link . '"' ) . ',"hide_notification_if_site_has_focus":true}, "data": {"conversation_id":"' . $conversation_id . '","user_id":"' . sb_get_active_user()['id'] . '"}}}';
...
function sb_send_sms($message, $to, $template = true) {
    $settings = sb_get_setting('sms');

    // Retrive phone number
    if ($to == 'agents' || strpos($to, 'department-') !== false) {
        $phones = sb_db_get('SELECT value FROM sb_users, sb_users_data WHERE id = user_id AND (user_type = "agent" OR user_type = "admin") AND slug = "phone"' . (strpos($to, 'department-') !== false ? ' AND department = ' . substr($to, 11) : ''), false);
        for ($i = 0; $i < count($phones); $i++) {
            sb_send_sms($message, $phones[$i]['value'], $template);
        }
        return false;
    ...
    $user = sb_db_get('SELECT id, first_name, last_name, user_type, email FROM sb_users, sb_users_data WHERE id = user_id AND slug = "phone" AND value = "' . $to . '" LIMIT 1');
    if (!sb_is_agent() && !sb_is_agent($user) && sb_isset(sb_get_active_user(), 'id') != sb_isset($user, 'id')) {
        return new SBError('security-error', 'sb_send_sms');
    }
    $message_template = $template ? sb_($settings[sb_is_agent() ? 'sms-message-user' : 'sms-message-agent']) : false;
    $message = $message_template ? str_replace('{message}', strip_tags($message), $message_template) : $message;
    $message = str_replace(['{recipient_name}', '{sender_name}', '{recipient_email}', '{sender_email}'], [sb_get_user_name($user), sb_get_user_name(), $user['email'], sb_isset(sb_get_active_user(), 'email', '')], sb_merge_fields($message));

    // Send the SMS
    $query = ['Body' => sb_clear_text_formatting($message), 'From' => $settings['send-smser'], 'To' => $to];
...
function sb_automations_validate($automation) {
    ...
    for ($i = 0; $i < count($conditions); $i++) {
        ...
        switch ($conditions[$i][0]) {
            ...
            case 'include_urls':
            case 'exclude_urls':
                $url =  str_replace(['https://', 'http://', 'www.'], '', $_SERVER['HTTP_REFERER']);
